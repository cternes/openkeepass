...
public class MetaBuilderTest {
	...
	public void shouldBuildEntryWithChangeDates() {
		Meta meta = new MetaBuilder("test").databaseDescriptionChanged(CalendarHandler.createCalendar(2015, 2, 5))
				...
				.recycleBinChanged(CalendarHandler.createCalendar(2015, 7, 6)).build();

		Assert.assertEquals("2015-02-05 00:00:00",
				CalendarHandler.formatCalendar(meta.getDatabaseDescriptionChanged()));
	...
	public void shouldBuildEntryFromExistingEntry() {
		Meta meta = new MetaBuilder("test").databaseDescription("abc")
				.databaseDescriptionChanged(Calendar.getInstance()).databaseNameChanged(Calendar.getInstance())
				.generator("openkeepass").historyMaxItems(10).historyMaxSize(15).maintenanceHistoryDays(20)
				.recycleBinChanged(Calendar.getInstance()).recycleBinEnabled(true).recycleBinUuid(UUID.randomUUID())
				.build();
