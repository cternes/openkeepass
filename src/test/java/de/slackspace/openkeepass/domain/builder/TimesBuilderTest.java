...
public class TimesBuilderTest {
	...
	public void shouldCheckEqualityOfObjects() {
		Times times = new TimesBuilder().creationTime(Calendar.getInstance()).expires(true)
				.expiryTime(Calendar.getInstance()).lastAccessTime(Calendar.getInstance())
				.lastModificationTime(Calendar.getInstance()).locationChanged(Calendar.getInstance()).usageCount(20)
				.build();
